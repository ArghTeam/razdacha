...
| `country` | string | не используется с [ADR 0018](decisions/0018-unified-pool.md); колонка оставлена, всегда пуста |
...
каталога, `pool` — снятые с него серверы (URL, подпись, пинг карточки). Такой
туннель разворачивается в N outbound'ов плюс `urltest` под тегом туннеля; ротацию и
health-check ведёт sing-box ([ADR 0010](decisions/0010-tunnel-pool-urltest.md)). Протоколы
внутри пула смешиваются: `urltest` собирает участников по каждой ссылке отдельно, так что
vless, shadowsocks и trojan живут в одной группе
([ADR 0018](decisions/0018-unified-pool.md)). Выключение пула — штатный
`enabled`. Колонку `pool` ведёт расписание, а не пользователь: изменившийся состав демон
применяет сам, не дожидаясь кнопки «Применить» ([Applying](05-api.md#applying)).
...
Заводит и держит его `store.EnsureBuiltinPool`, идемпотентно по паре (`builtin`,
`source = pool`): встроенный пул уже есть — он и остаётся, нет — создаётся выключенным,
иначе свежая установка сразу пошла бы за ключами, о чём никто не просил. Апгрейд с версии
со странами ([ADR 0017](decisions/0017-country-default-tunnels.md)) сворачивает семь
страновых пулов в один: лишние удаляются, а ссылавшиеся правила отвязываются **не молча** —
у первого звена `tunnel_id` уходит в `NULL` и правило выключается, у второго `via_tunnel_id`
уходит в `NULL` с сохранением правила ([ADR 0013](decisions/0013-unavailable-tunnel-rejects.md)).
Схема при этом не меняется — сворачивание идёт стартовым реконсилом, а не шагом миграции.

Своих пулов пользователь не создаёт: `POST /api/tunnels` со ссылкой на каталог отвечает
`409`. Признак хранится колонкой, а не выводится из совпадения `raw` с каталогом:
пользователь может завести свой туннель по тому же адресу, а адрес источника может
поменяться в релизе — и оба случая перепутали бы «своё» с «встроенным». Встроенная запись
не удаляется (`DELETE` отвечает `409`) — её выключают; включение, выключение и `refresh`
работают у каждой как у любого туннеля.
...
CREATE TABLE tunnels (
  ...
  country TEXT NOT NULL DEFAULT '', builtin INTEGER NOT NULL DEFAULT 0);
...
**Копия состояния — ключи `backup_enabled`, `backup_interval`, `backup_passphrase`,
`backup_last_sent_at`, `backup_last_error` там же и тоже вне `Settings`.**
`backup_interval` — секунды, меньше часа не принимается; включить отправку без
парольной фразы нельзя вовсе ([ADR 0016](decisions/0016-backup-full-and-encrypted.md)):
это условие работы расписания, а не галочка «зашифровать». Фраза лежит в открытом виде —
иначе ей нечего было бы шифровать копию; это та же категория секрета, что токен бота, и
живёт она в том же файле с правами `0600`, где и приватные ключи пиров. Наружу фраза не
отдаётся: `GET /api/backup` возвращает только признак `passphrase_set`.
