...
| `country` | string | ISO-код страны встроенного странового пула (`NL`, `DE`, …); пусто у остальных туннелей |
...
vless, shadowsocks и trojan из одной страны живут в одной группе
([ADR 0017](decisions/0017-country-default-tunnels.md)). Выключение пула — штатный
...
**`builtin = 1`** — запись, которую демон заводит сам. Таких **семь** — по одному
встроенному пулу на захардкоженную страну: 🇳🇱 NL, 🇩🇪 DE, 🇺🇸 US, 🇫🇷 FR, 🇬🇧 GB, 🇫🇮 FI,
🇰🇿 KZ ([ADR 0017](decisions/0017-country-default-tunnels.md)). Каждый — обычный пул
(`source = pool`, `enabled = 0` по умолчанию) со своим `country`; имя строит
`store.Country.PoolName()` — флаг плюс название страны. Выбор выхода становится выбором
страны: снаружи каждый пул остаётся одним туннелем с одной группой `urltest`, а ротация
на живой-быстрый внутри страны бесплатна ([ADR 0010](decisions/0010-tunnel-pool-urltest.md)).

Страна берётся **офлайн geo-IP базой** (DB-IP Country Lite, `internal/geoip`) по
exit-адресу сервера, а не по метке каталога: домен резолвится, IP отдаёт страну. Метки
источника врут, поэтому не используются принципиально.

Заводит пулы `store.EnsureBuiltinCountryPools`, идемпотентно по паре (`builtin`,
`country`): пул нужной страны уже есть — он и остаётся, нет — создаётся выключенным, иначе
свежая установка сразу пошла бы за ключами, о чём никто не просил. Единственный
бесстрановой встроенный пул прежних версий ([ADR 0015](decisions/0015-pool-source-driver.md))
упразднён: миграция 11 его удаляет, а ссылавшиеся правила отвязывает **не молча** — у
первого звена `tunnel_id` уходит в `NULL` и правило выключается, у второго `via_tunnel_id`
уходит в `NULL` с сохранением правила.
