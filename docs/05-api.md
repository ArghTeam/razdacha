...
его закрывает nginx на `10.8.0.1:443` ([ADR 0008](decisions/0008-nginx-before-panel.md)).
Доступ только из VPN, поэтому без аутентификации
([архитектура](01-architecture.md#модель-привилегий)); TLS терминируется на nginx, до
