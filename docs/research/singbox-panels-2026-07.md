...
Третий вопрос: у нас модалка с каталогом списков, поиском, приоритетом стрелками и
предупреждением о перекрытии — кто нашёл форму лучше и в чём именно. Смотрелись веб-панели
(3x-ui, s-ui, Marzban, Remnawave, Hiddify Manager), роутерные пакеты (Podkop, homeproxy,
Passwall/Passwall2, OpenClash, nikki) и клиенты, где редактор правил самый зрелый (Throne —
живой форк nekoray, NekoBox for Android, v2rayN, Clash Verge Rev, FlClash, Karing,
sing-box-for-android, Shadowrocket). Состояние репозиториев — конец июля 2026; ссылки на
Podkop проверены грепом в `~/Web/podkop`, остальные — чтением исходников на GitHub, где
проверить не удалось, сказано отдельно.

Три поправки к исходным вводным, чтобы не искать несуществующее: `MatsuriDayo/nekoray`
заброшен, поддерживаемый форк — `throneproj/Throne`; `xiaorouji/openwrt-passwall` переехал в
организацию `Openwrt-Passwall`; репозиторий `remnawave/panel` — это сайт документации, кода
панели в открытом доступе нет, поэтому про Remnawave всё сказанное ниже — по доке.

### Что кладётся в правило

Четыре разных ответа, и они выстраиваются в лестницу от «поле на каждый атрибут» до «пишите
JSON сами».

**Типизированная форма.** homeproxy — прямое отображение схемы sing-box на LuCI: `domain`,
`domain_suffix`, `domain_keyword`, `domain_regex` — четыре отдельных поля, а не один текстарий
с префиксами, плюс `ip_cidr`, порты, `process_name`/`process_path`, `invert`
(`htdocs/luci-static/resources/view/homeproxy/client.js:805-866`), разложенные по вкладкам
внутри модалки (`:580-583`). s-ui той же природы, но собирается наоборот: форма стартует
пустой, и через меню «Options» пользователь включает нужные группы условий, а режим совпадения
(`domain`/`domain_suffix`/`domain_keyword`/`domain_regex`/`ip_cidr`) — отдельная выпадашка,
переключение которой стирает остальные ключи (`s-ui-frontend`, `src/components/Rule.vue:324`,
`:355-358`).

**Многострочный ввод с префиксами.** Passwall: два больших textarea — домены и IP, а тип
совпадения кодируется префиксом строки (`regexp:`, `domain:`, `full:`, `geosite:`,
`rule-set:remote:`), с примерами прямо в описании поля
(`luci-app-passwall/luasrc/model/cbi/passwall/client/shunt_rules.lua:158-160`, `:195-209`).
Throne — то же самое, набор префиксов задокументирован одной константой
(`include/global/Const.hpp:31`). Дёшево в реализации, но синтаксис приходится знать наизусть.

**Голый текст.** OpenClash: два textarea, куда руками пишется `DOMAIN-SUFFIX,youtube.com,🚀
Proxy` (`luci-app-openclash/luasrc/model/cbi/openclash/config-overwrite.lua:463-507`).
Показательно, что авторы умеют делать сортируемые таблицы и делают их для DNS-серверов и
групп (`:513`), но не для правил. Marzban ушёл ещё дальше: редактора правил нет вовсе, весь
конфиг Xray правится как JSON-блоб (`app/dashboard/src/components/CoreSettingsModal.tsx:261`).
Hiddify Manager — противоположная крайность: маршрутизация зашита в шаблон
`xray/configs/03_routing.json.j2`, наружу торчат три настройки (`warp_mode`, `warp_sites`,
`country`), и сценарий «банк мимо туннеля» в интерфейсе **не выражается вообще** — второго
назначения нет.

**Каталог готовых списков — почти ни у кого.** Это главная находка по первому подвопросу.
Каталог с поиском есть ровно у одного из пятнадцати: Karing, поле «Rule Set (build-in)» с
пикером (`lib/screens/diversion_group_custom_edit_screen.dart:337-388`), который открывает
`MultiSelectScreen` с живым поиском и кнопкой очистки (`lib/screens/multi_select_screen.dart:43-84`).
Throne подставляет автодополнение по 2188 записям, вшитым в заголовок `srslist.h` на этапе
сборки (`.github/workflows/build.yml:210-246`) — листать нельзя, только угадывать начало.
nekoray дополнял категориями прямо из `geosite.dat`
(`3rdparty/qv2ray/v3/components/GeositeReader/GeositeReader.cpp:12-40`). Clash Verge Rev даёт
Autocomplete по 33 **типам** правил с примером и валидатором на каждый
(`src/components/profile/rules-editor-viewer.tsx:72-233`), но сами категории geosite —
свободный текст. У остальных — ничего: homeproxy, nikki, OpenClash, s-ui требуют завести
ruleset руками, вписав URL `.srs` (homeproxy `client.js:1311-1396`,
`luci-app-nikki/htdocs/luci-static/resources/view/nikki/mixin.js:461-528`), а v2rayN вместо
каталога ставит гиперссылку на документацию Xray
(`v2rayN/v2rayN/Views/RoutingRuleDetailsWindow.xaml:170-173`).

Podkop, наш источник, — посередине: `community_lists` это `DynamicList` из справочника на 23
позиции, вшитого в UI (`luci-app-podkop/htdocs/luci-static/resources/view/podkop/main.js:896-921`,
поле — `section.js:296-306`), пользовательские домены вводятся в двух режимах на выбор,
построчно или одним текстом (`section.js:384-432`), подсети так же (`:466-530`), плюс
локальные и удалённые `.srs` по URL (`:545`, `:591`, `:618`). Наш каталог отличается тем, что
приезжает с сервера и ищется поиском (`ui/dist/js/screens/rules.js:186-198`), — по этому
пункту мы вторые после Karing.

### Приоритет и перекрытие

Приоритет везде одинаков и скучен: порядок строк, первое совпадение выигрывает. Разница
только в способе двигать. Drag-n-drop — homeproxy (`client.js:574`, `sortable = true`),
Passwall со своим шаблоном на SortableJS (`luasrc/view/passwall/cbi/sortable.htm`, включается
в `client/rule.lua:152-177`), nikki (`mixin.js:539`), s-ui на нативном HTML5-DnD
(`src/views/Rules.vue:127-129`), FlClash, NekoBox for Android (`ui/RouteFragment.kt:48-91`).
Кнопки «выше/ниже» — Throne (`src/ui/setting/RouteItem.cpp:680-696`) и v2rayN, у которого
это контекстное меню с горячими клавишами (`ConfigHandler.cs:2218-2286`). 3x-ui делает всё
сразу: ручка перетаскивания, «вверх/вниз» в меню строки и номер `index + 1`
(`frontend/src/pages/xray/routing/useRoutingColumns.tsx:51-91`).

Номер правила на экране — редкость. Его нет у homeproxy, Passwall, nikki, Throne, v2rayN,
FlClash, Karing; у Clash Verge Rev он есть, но только на read-only странице просмотра правил
(`src/pages/rules.tsx:39-43`), а в редакторе исчезает. Из панелей номер показывают 3x-ui и
s-ui (заголовок карточки `index+1`, `Rules.vue:130`).

**Перекрытие не показывает никто.** Это подтверждено грепом по всем пятнадцати проектам: нет
ни одного места, где два правила сравнивались бы между собой или где правило помечалось бы
недостижимым. Суррогаты, которые встретились:

- Passwall пишет красным в заголовке секции «Please note attention to the priority, the higher
  the order, the higher the priority» (`client/rule.lua:152`) — напоминание вместо проверки;
- Karing держит на экране две вечные подсказки: «Try to match the rules from top to bottom.
  If no rule is matched, use [final]» и, при добавлении правила, «After adding, you may need
  to manually adjust the order, otherwise the newly added diversion may not take effect»
  (`lib/screens/diversion_rules_screen.dart:107-116`);
- Podkop проверяет взаимоисключение внутри одного поля: выбор второго регионального списка или
  `russia_inside` вместе с сервисом, который в него уже входит, снимает лишнее и показывает
  уведомление (`section.js:311-382`, наборы — `main.js:875-895`). Это конфликт списков внутри
  одной секции, а не перекрытие двух правил;
- homeproxy ловит рекурсию в цепочке outbound'ов («Recursive outbound detected!»,
  `client.js:473-490`) — другая проблема, но единственное место во всей выборке, где слово
  conflict вообще встречается;
- v2rayN вставляет новое правило в позицию 0 (`RoutingRuleSettingViewModel.cs:146`), то есть
  по умолчанию затеняет им всё остальное и молчит об этом.

Наш расчёт перекрытия — с учётом суффиксов доменов и вложенности подсетей, в строке списка и
живым в форме (`ui/dist/js/screens/rules.js:32-72`, `:363-380`) — в этой выборке уникален.
Ниша открытая, и это единственное место, где мы впереди рынка, а не догоняем.

### Сколько шагов до «ютуб в туннель, банк мимо»

Дешевле всех — Karing, около трёх тапов, и не из-за формы, а из-за модели: «что матчится»
(именованная группа условий) отделено от «куда идёт» (`DiversionRulesScreen`, строка-группа →
`ServerSelectScreen`, `lib/screens/diversion_rules_screen.dart:246-269`). Встроенные
geosite-группы уже перечислены, YouTube среди них, так что половина задачи — один тап.
Банк требует завести свою группу и потом отдельно назначить ей направление — восемь действий
на двух экранах. Дальше по дешевизне: nikki — две строки таблицы на одной странице
(`mixin.js:533-568`); homeproxy — две модалки по три поля, плюс готовые режимы
`gfwlist`/`bypass_mainland_china`/`global` для типовых случаев (`client.js:251-256`);
3x-ui — две модалки, перетаскивание и сохранение; Clash Verge Rev — два раза «Prepend Rule»
и Save. Дороже всех Passwall: правила заводятся на одной странице, а назначение им ставится
на другой, в таблице `shunt_options` (`client/include/shunt_options.lua`), — шесть-восемь
действий на трёх экранах. У Hiddify Manager задача не решается в интерфейсе вовсе.

Пресеты, которые реально сокращают путь, есть у двоих. v2rayN: три встроенных набора
(`ConfigHandler.InitBuiltinRouting`, `ConfigHandler.cs:2428-2491`), подписка на наборы правил
по URL (`:2365-2419`) и региональные пресеты, включая российский
(`ApplyRegionalPreset`, `:2723-2786`, источники — `Global.cs:188-193`). Karing: региональные
пресеты, где каждое правило — строка-переключатель «имя → аутбаунд»
(`lib/screens/diversion_rules_custom_set_screen.dart:202-253`). Throne пресетов не имеет
совсем, зато принимает чужие профили маршрутизации по ссылке с автообновлением
(`src/ui/setting/RouteItem.ui:68-135`), и готовые лежат в `throneproj/routeprofiles`
(`Bypass_Russia`, `Proxy_Antizapret`, `Proxy_Refilter`). У nekoray «пресет» — две кнопки,
которые перезаписывают все шесть полей разом (`ui/dialog_manage_routes.cpp:113-139`).

### Что видно, когда правило не работает

Самый интересный подвопрос, потому что здесь разброс максимальный, а большинство молчит.

**Мёртвое назначение.** FlClash красит цель правила прямо в списке: DIRECT зелёным, REJECT
оранжевым, а неизвестную цель — красным, с кнопкой ⓘ и текстом «{target} is an invalid
policy» (`lib/features/overwrite/rule.dart:37-62`, строки — `arb/intl_en.arb:515-516`).
Karing так же красит строку красным, сверяя тег с живым списком аутбаундов
(`lib/screens/diversion_rules_screen.dart:235-262`). Throne показывает в выпадашке
«INVALID OUTBOUND» (`src/ui/setting/RouteItem.cpp:36-42`) и отказывается собирать конфиг
целиком (`src/configs/generate.cpp:193`). Всё остальное молчит: homeproxy просто фильтрует
выключенные ноды из выпадашки (`client.js:677`) и молча выбрасывает их при генерации
(`root/etc/homeproxy/scripts/generate_client.uc:902,945`), так что ссылка на выключенное
остаётся висеть невидимой; Passwall позволяет выбрать мёртвую ноду без предупреждения, а пинг
показывает на другой странице (`view/passwall/node_list/node_list.htm:556-641`); s-ui не
проверяет ничего; v2rayN — худший случай: неизвестный тег молча подменяется на `proxy`
(`ServiceLib/Services/CoreConfig/SingboxRoutingService.cs:550-564`, то же в
`V2rayRoutingService.cs:180-194`), то есть трафик уходит не туда, куда написано в правиле, и
об этом не говорится нигде.

Отдельно стоит 3x-ui: он не показывает поломку постфактум, а не даёт её создать. Удаление
outbound'а считает `DeletionImpact` — какие правила потеряют тег, а какие останутся без
назначения вовсе, — и показывает список в модалке подтверждения
(`frontend/src/pages/xray/reference-cleanup.ts:86-99`, `:211-224`;
`frontend/src/pages/xray/DeletionImpactList.tsx:12-24`). Обоснование записано в коде дословно:
висячий `balancerTag` роняет `Router.Init`, а висячий `outboundTag` отправляет совпавший
трафик в чёрную дыру.

**Список не скачался.** Тут почти пусто. Karing — единственный с отдельным экраном состояния:
таблица из `getRemoteRulesetsStates` со временем последнего обновления и текстом ошибки на
каждый ruleset, плюс сводка «Downloading or download failed» для тех, у кого ноль записей
(`lib/screens/diversion_rule_detect_screen.dart:243-325`, `:405-420`). Clash Verge Rev
показывает по провайдеру число правил и относительное время обновления
(`src/components/rule/provider-button.tsx:212-244`), но состояние ошибки не хранит, а
«Update All» глотает сбои и всё равно рапортует успех (`:100-115`). homeproxy показывает
версию и цвет, но только для четырёх встроенных списков и не для пользовательских ruleset'ов
(`view/homeproxy/status.js:107-109`). OpenClash выдаёт за версию mtime файла
(`luasrc/model/cbi/openclash/settings.lua:1029`). Passwall — только бегущий лог после ручного
обновления (`luasrc/controller/passwall.lua:755-763`). nikki, s-ui, Throne, v2rayN — ничего.
Podkop состояние списков в правиле не показывает тоже: неработоспособность видна на отдельной
вкладке диагностики пятью общими проверками — DNS, Sing-box, Nftables, Outbounds, FakeIP
(`main.js:1115-1140`).

**«Какое правило сработает для этого домена».** Проактивных пробников два, и оба стоят
внимания. 3x-ui: `RouteTester` шлёт назначение, порт, сеть и протокол на `/panel/api/xray/routeTest`
и спрашивает у **работающего ядра**, какой outbound победит; зелёная плашка с тегом или
янтарная «уходит в outbound по умолчанию» (`frontend/src/pages/xray/routing/RouteTester.tsx:45`,
`:129-150`). Karing: поле «домен» и кнопка Detect, за которой поднимается VPN, домен при
необходимости резолвится, и `outboundQuery` возвращает **и сработавшее правило, и цепочку
аутбаундов, и статус загрузки его rule set** (`lib/screens/diversion_rule_detect_screen.dart:161-241`).
Всё остальное отвечает на этот вопрос только задним числом, через список соединений: Clash
Verge Rev показывает и колонку `rule`, и колонку `chains`
(`src/components/connection/connection-table.tsx:421-431`), FlClash печатает совпавшее правило
в детали соединения (`lib/views/connection/item.dart:181-187`), sing-box-for-android — «Match
rule» и цепочку (`ConnectionDetailsScreen.kt:223-243`). v2rayN и Throne показывают только
цепочку аутбаундов, поля правила в их модели соединения нет
(`include/stats/connections/connectionLister.hpp:31-41`).

### Сводка

| | каталог с поиском | типы условий | порядок | номер | перекрытие | мёртвая цель | статус списков | пробник |
|---|---|---|---|---|---|---|---|---|
| razdacha | **да** | списки + домены + подсети | стрелки | **да** | **да** | нет | нет | нет |
| Podkop | справочник без поиска | списки + домены + подсети | порядок секций | нет | внутри поля | нет | нет | нет |
| 3x-ui | только на вкладке Basic | CSV в полях | DnD + меню + номер | да | нет | **превью удаления** | нет | **да** |
| s-ui | нет | режимы, textarea | DnD | да | нет | нет | нет | нет |
| homeproxy | нет | **отдельное поле на тип** | DnD | нет | нет | прячет из выпадашки | 4 встроенных | нет |
| Passwall | нет | префиксы в textarea | DnD | нет | красная фраза | нет | лог | нет |
| OpenClash | нет | голый текст | строки файла | нет | нет | нет | mtime | нет |
| nikki | нет | тип + одно поле | DnD | нет | нет | нет | нет | нет |
| Throne | автодополнение 2188 | **вкладки-атрибуты** | кнопки | нет | нет | «INVALID OUTBOUND» | нет | нет |
| v2rayN | нет | три textarea | меню + горячие | нет | нет | **молча → proxy** | нет | нет |
| Clash Verge Rev | по типам правил | тип из 33 + валидатор | DnD (частично) | только просмотр | нет | выбор из живых | время обновления | нет |
| FlClash | нет | тип из 34 с описанием | DnD | нет | нет | **красным + ⓘ** | нет | нет |
| Karing | **да, с поиском** | поле на тип | DnD групп | нет | подсказки | **красным** | **да, с ошибкой** | **да** |
| Marzban / Remnawave / Hiddify Manager | — | JSON или шаблон | — | — | — | — | — | — |

### Что стоит перенять

- **Пробник маршрута** (3x-ui `RouteTester.tsx`, Karing `diversion_rule_detect_screen.dart`) —
  единственный прямой ответ на «почему не работает»; у нас Clash API уже есть, спросить у
  живого ядра, какой outbound победит для домена, дешевле, чем объяснять порядок словами.
  Тот же пункт назван выше по другому поводу — это одна работа, а не две.
- **Статус каждого списка рядом с правилом** (Karing: время обновления и текст ошибки на
  ruleset) — наш каталог обновляется сам, и когда он не обновился, правило тихо перестаёт
  ловить, а панель показывает тот же чип, что и в рабочем состоянии.
- **Красная цель у правила, когда туннель выключен или пул пуст** (FlClash `rule.dart:37-62`) —
  по [ADR 0013](../decisions/0013-unavailable-tunnel-rejects.md) такое правило становится
  `reject`, то есть ресурс перестаёт открываться, а в списке правил это выглядит ровно как
  рабочее: мы читаем только имя туннеля (`ui/dist/js/screens/rules.js:84-91`), а `enabled` и
  `status` у нас на руках уже есть.
- **Номер правила на экране редактирования, а не только в списке** (3x-ui `index + 1`) — у нас
  позиция объясняется фразой в шапке модалки, но цифры рядом с полем «Куда» нет, а именно она
  отвечает на «почему сработало другое».
- **Региональные пресеты как набор строк «список → направление»** (Karing
  `diversion_rules_custom_set_screen.dart:202-253`, v2rayN `ApplyRegionalPreset`) — у нас
  первый запуск даёт пустой экран правил, а типовая раскладка «эти списки в туннель, банки
  напрямую» одинакова почти у всех и собирается из уже имеющегося каталога.
- **Отчёт об обновлении списков со списком отказавших** (Throne
  `dialog_manage_routes.cpp:554-598`) — против образца «Update All глотает ошибки и рапортует
  успех» у Clash Verge Rev (`provider-button.tsx:100-115`).

### Что мы делаем иначе намеренно

- **Дефолты вместо опций** (`context/ui/invariants.md`). Throne показывает 31 атрибут правила
  через вкладки, homeproxy — четыре отдельных поля под домен и ещё четыре под порты, s-ui даёт
  логические правила с `invert`. У нас условий три вида — списки, домены, подсети, — и это
  не бедность формы, а следствие того, что клиент не настраивает ничего: правило обязано быть
  понятно тому, кто не знает слова `domain_keyword`.
- **Каталог приезжает с сервера, а не вшит в интерфейс.** У Podkop справочник из 23 позиций
  лежит константой в `main.js`, у Throne — заголовком на 2188 записей, собираемым при сборке.
  Наш каталог тянется из API и потому обновляется без релиза панели, а поиск включается только
  когда позиций больше восьми (`rules.js:149`).
- **Стрелки, а не drag-n-drop.** Перетаскивание есть у большинства и выглядит современнее, но
  у нас порядок отправляется целиком одним запросом (`rules.js:483-499`), список короткий, а
  стрелки работают с клавиатуры и не требуют ни библиотеки, ни возни с pointer-событиями —
  3x-ui на это потратил полсотни строк с `document.elementFromPoint`.
- **Никакого сырого JSON и никакого escape hatch.** Marzban, Remnawave, s-ui и
  sing-box-for-android рано или поздно отдают пользователю текстовое поле с конфигом. У нас
  конфиг генерируется целиком из БД и не патчится — редактор JSON противоречил бы этому
  инварианту, а не дополнял его.
- **Одно действие вместо цепочки экранов.** Двухуровневая модель Karing («что матчится»
  отдельно от «куда идёт») даёт три тапа на типовой задаче, но ценой второй сущности и второго
  экрана. У нас правило — одна сущность с назначением внутри
  ([ADR 0003](../decisions/0003-tunnel-separate-from-rule.md) развязывает туннель и правило, но
  не матчинг и направление), и путь до «ютуб в туннель, банк мимо» — две модалки, как у 3x-ui
  и homeproxy, без хождения между страницами, как в Passwall.
