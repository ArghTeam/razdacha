...
### #152 — `last_ok` в ответе туннелей
**Changed:** `GET /api/tunnels` отдаёт `last_ok` (`*time.Time`, `null` — удачных проверок нет); `okStamp()` решает удачу (`up`/`slow`), `checkCache.put` переносит отметку через неудачные круги.
**Beware:** ручная `POST /api/tunnels/{id}/check` в БД не пишет — удача живёт в кэше демона до перезапуска, потом дата откатывается к прежнему успеху; ошибка в безопасную сторону.

### #153 — тип туннеля пула для API решает драйвер каталога
**Changed:** `tunnelType()` спрашивает `lists.PoolKeyType(url)` при `PATCH` пула и в превью `POST /api/tunnels/parse`; неизвестный хост каталога отвечает `400`.
**New surface:** `servers[].ping_ms` в ответах туннелей стал `*int`, `null` — источник пинг не даёт.
**Beware:** ссылка на неизвестный каталог отказывает до сохранения — `400`, а не тихий дефолтный тип.

### #156 — 409 на занятый каталог пула
**Changed:** `poolCrawlError` мапит `lists.ErrNoPoolDriver`/`store.ErrInvalid` в `400 bad_request`, `lists.ErrPoolCrawlBusy` в `409 conflict`, остальное — `502 internal`.
**Beware:** Раньше неизвестный адрес каталога отвечал `502`, хотя причина — ввод пользователя.
