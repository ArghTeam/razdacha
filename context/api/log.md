...
**Beware:** `POST /api/apply` перезаливает только конфиг sing-box, nft не трогает — «применено» в панели неправда для подсетей (#119). Диагностика отвечает `ok` ровно в состояниях утечки: выпавшее правило это штатный успех `Generate`, устаревший сет ни с чем не сверяется (#123). Инвариант «ссылки `vless://` наружу не отдаются» соблюдён в составе пула и нарушен в списке туннелей: `tunnelResponse.Raw` — это исходный ввод пользователя, для WARP там `PrivateKey` (#124).
