...
**Beware:** `route.final = direct-out` делает пропуск правила утечкой, а не отказом: домены теряют FakeIP, подсети остаются в nft-сете и доезжают до sing-box, который выпускает их сам. Инвариант слоя фиксировал пропуск как замысел — обоснование верное, следствие нет; заменяется ADR 0013 (#130). Отдельно: `allowInsecure` из ключа публичного каталога снимает проверку сертификата (#128).
