...
Есть встроенные туннель-пулы — по одному на страну (🇳🇱 NL, 🇩🇪 DE, 🇺🇸 US, 🇫🇷 FR, 🇬🇧 GB,
🇫🇮 FI, 🇰🇿 KZ, [ADR 0017](docs/decisions/0017-country-default-tunnels.md)): выбор выхода —
это выбор страны. Внутри пула сервер сам берёт бесплатные ключи, держится за живой и
переключается без перезапуска ([ADR 0010](docs/decisions/0010-tunnel-pool-urltest.md)).
Заводятся выключенными — пока их не включили, наружу за ключами никто не ходит.
