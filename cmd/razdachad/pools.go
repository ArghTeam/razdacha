...
// igareckCatalogURL — источник ключей встроенных страновых пулов (ADR 0017). Один
// адрес на все семь стран: страны различает колонка country, а обходит каталог и
// отбирает по ней ключи драйвер слоя lists (задача #170). Базовый адрес — зеркало
// CDN githack поверх репозитория igareck/vpn-configs-for-russia; точные файлы и
// разбор — за драйвером, хранилищу достаточно строки, чтобы пул был засеян.
const igareckCatalogURL = "https://raw.githack.com/igareck/vpn-configs-for-russia/main/"

// poolCatalogType — тип встроенных пулов. Косметичен: участники группы urltest
// разбираются каждый по своей ссылке (ADR 0010/0015), настоящий протокол ключа
// приходит из каталога. Разумный дефолт до драйвера #170 — vless: конфиги igareck
// в основном vless.
const poolCatalogType = store.TunnelVLESS

// ensureBuiltinPool приводит БД к состоянию «семь встроенных страновых пулов есть»
// (ADR 0017). Заведены они выключенными: свежая установка не должна сама ходить на
// чужой сайт за ключами. Старый единственный пул, оставшийся от прежних версий,
// удаляется, а ссылавшиеся на него правила отвязываются — про каждое пишется в лог,
// потому что снаружи такая правка БД ничем себя не проявляет.
//
// Неудача демон не останавливает: без пулов панель работает, а сообщение в логе
// объясняет, почему их нет (например, имя пула занято туннелем пользователя).
func ensureBuiltinPool(ctx context.Context, st *store.Store, log *slog.Logger) {
	res, err := st.EnsureBuiltinCountryPools(ctx, igareckCatalogURL, store.CountryPools(), poolCatalogType)
	if err != nil {
		if ctx.Err() == nil {
			log.Warn("встроенные страновые пулы не заведены", "ошибка", err)
		}
		return
	}
	if len(res.Created) > 0 {
		log.Info("заведены встроенные страновые пулы",
			"страны", res.Created, "каталог", igareckCatalogURL)
	}
	for _, name := range res.RemovedLegacy {
		log.Warn("старый единственный встроенный пул удалён, его заменяют страновые",
			"туннель", name)
	}
	for _, r := range res.DetachedRules {
		if r.Disabled {
			log.Warn("правило ссылалось на удалённый пул и выключено", "правило", r.Name)
		} else {
			log.Warn("у правила отвязано второе звено цепи на удалённый пул", "правило", r.Name)
		}
	}
}
