...
func TestSummaryPublicExternalAddrUnknown(t *testing.T) {
	...
	for _, want := range []string{
		"Панель отвечает на всех интерфейсах",
		"внешний адрес",
		"определить не удалось",
		"Если внешний адрес сервера известен вам, панель открыта и по нему.",
	} {
		if !strings.Contains(out, want) {
			t.Fatalf("в выводе нет %q:\n%s", want, out)
		}
	}
	// Панель отвечает на всех интерфейсах — вывод не должен утверждать
	// обратное только из-за того, что адрес назвать нечем (неизвестен адрес,
	// а не доступ).
	if strings.Contains(out, "недоступна") {
		t.Fatalf("вывод утверждает недоступность снаружи, а панель отвечает на всех интерфейсах:\n%s", out)
	}
