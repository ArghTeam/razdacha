...
func (s summary) Render() (string, error) {
	...
	case s.PanelPublic:
		...
		// выписан на один адрес VPN. Панель при этом всё равно отвечает на всех
		// интерфейсах: неизвестен не доступ, а адрес, которым его назвать. Врать
		// про недоступность нельзя — со стенда видно 200 по внешнему IP, пока
		// вывод говорил обратное.
		line("Панель отвечает на всех интерфейсах, но её внешний адрес")
		line("определить не удалось — по адресу известен только VPN:")
		line("  %s", s.PanelURL)
		line("Если внешний адрес сервера известен вам, панель открыта и по нему.")
	default:
