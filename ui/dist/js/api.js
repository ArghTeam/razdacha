...
/* --- Резервная копия состояния --------------------------------------------
   Парольная фраза наружу не отдаётся: в ответе только `passphrase_set`. Пустая
   строка в запросе означает «стереть», отсутствие поля — «оставить прежнюю».

   Скачивание сюда не входит намеренно: файл бинарный и весит мегабайты, его
   тянет сам браузер по ссылке `DOWNLOAD_PATH` — cookie сессии HttpOnly и
   уезжает с обычным переходом, а держать копию базы в памяти вкладки незачем. */

export const backup = {
  get: () => get('/api/backup'),
  save: (body) => put('/api/backup', body),
  send: () => post('/api/backup/send'),
  DOWNLOAD_PATH: '/api/backup/download',
};
...
/* --- пробник маршрута -----------------------------------------------------
   Отвечает работающий sing-box, а не база: демон спрашивает у него правила
   маршрутизации и его же резолвер. Ядро не отвечает — приходит 503 с
   объяснением, и экран показывает его как есть: пустой результат читался бы
   как «ни одно правило не сработает». */

export const route = {
  test: (domain) => post('/api/route/test', { domain }),
};

/* --- проба доступности домена --------------------------------------------
   Сосед пробника маршрута, но вопрос другой: не «какое правило поймает домен»,
   а «открывается ли он напрямую, с адреса сервера, или отдаёт геоблок». Демон
   ходит к домену обычным HTTP со своего адреса и отдаёт `{ domain, status,
   class, verdict }`; вердикт на русском показывается как есть. */

export const domain = {
  reachability: (name) => post('/api/domain/reachability', { domain: name }),
};
