...
const (
	...
	// порядке приоритета, который ведёт слой lists. Каталог igareck живость ключа не
	// отражает (ключ — строка в подписке, «присутствует» всегда), поэтому мёртвые по
	// urltest держат слоты, и узкое окно оставляет живой резерв пустым: на стенде из
	// 16 в окне отвечали 2. Шестьдесят четыре дают urltest, из чего выбирать живого, а
	// цена мала — 64 записи в outbounds[] и 64 пробы каждые PoolTestInterval (пробы
	// внутри процесса, конфиг не меняется, соседи не задеты).
	poolMaxServers = 64
