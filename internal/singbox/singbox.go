// Package singbox генерирует конфиг sing-box из состояния БД.
//
// Генерация — чистая функция: [Generate] принимает снимок состояния и возвращает
// [option.Options] целиком. Частичного патча конфига нет и не будет: правило,
// ссылающееся на туннель, и DNS-правило того же набора обязаны меняться вместе,
// а точечная правка JSON этого не гарантирует.
package singbox
