...
// tunnelNameMax — предел имени туннеля в клиенте WireGuard для Android:
// имя берётся из имени файла без расширения и валидируется как
// `[a-zA-Z0-9_=+.-]{1,15}`. Всё, что длиннее, клиент отказывается импортировать
// со словами «неправильное имя», не объясняя, что дело в длине.
const tunnelNameMax = 15

// cyrillicTranslit — транслитерация для имён пиров: они у нас русские, а в имя
// туннеля кириллица не проходит. Выбрасывать её нельзя — «Телефон Ромы» и
// «Телефон Ани» схлопнулись бы в одно имя.
...
// confFileName — имя файла для скачивания. Кириллица транслитерируется, всё
// прочее за пределами разрешённого алфавита схлопывается в дефис, результат
// режется по tunnelNameMax: имя пира вводит пользователь, а клиент на телефоне
// молча отказывает на всём, что не укладывается в его регулярку.
//
// Префикса у имени нет намеренно: он съедал бы больше половины бюджета в 15
// символов, а в списке туннелей на телефоне ничего не различал.
func confFileName(name string) string {
	var b strings.Builder
	prev := false
	for _, r := range strings.ToLower(name) {
		if t, ok := cyrillicTranslit[r]; ok {
			// Мягкий и твёрдый знаки дают пустую строку — это не разделитель,
			// но и не символ: «Русь» должно стать «rus», а не «rus-».
			if t != "" {
				b.WriteString(t)
				prev = false
			}
	...
	if len(out) > tunnelNameMax {
		// Обрезка идёт по байтам, но после транслитерации в строке только ASCII.
		out = strings.Trim(out[:tunnelNameMax], "-")
	}
	if out == "" {
		out = "peer"
	}
