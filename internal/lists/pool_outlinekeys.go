...
//
// Это подпись чужого сайта, а не измерение: сверка показала расхождение с
// геолокацией и реестром на одном ключе из пяти проверенных (issue #161).
// `store.PoolServer.Country` остаётся тем же полем и тем же именем — это
// по-прежнему то, что отдал каталог, — но панель обязана показать его как
// подпись источника, а не как проверенный факт.
